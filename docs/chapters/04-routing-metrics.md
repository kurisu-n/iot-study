...
Ένα σημείο που μπερδεύει εύκολα: αυτό **δεν** είναι «πήγαινε-έλα». Και οι τρεις όροι αφορούν **μία** μετάδοση προς **μία** κατεύθυνση, απλώς μοιρασμένη σε **δύο κόμβους**. Το \(E_c\) εμφανίζεται δύο φορές όχι επειδή στέλνουμε δύο φορές, αλλά επειδή πληρώνουν τα ηλεκτρονικά τους **και οι δύο άκρες** της ίδιας μετάδοσης: ο πομπός για να στείλει, ο δέκτης για να λάβει. Ο ενισχυτής \(E_{tx}(d)\) μπαίνει **μία** φορά, γιατί μόνο ο πομπός «σπρώχνει» το σήμα· ο δέκτης απλώς ακούει, δεν ενισχύει τίποτα. Δεν χρειάζεται λοιπόν κανένα ×2. Το «πήγαινε-έλα» του ACK κρύβεται μέσα στο ίδιο το ETX (μέσα από το \(d_r\), την πιθανότητα της επιστροφής), και η ελάχιστη ενέργειά του αγνοείται σε αυτό το απλό μοντέλο.
