...
Χρειαζόμαστε λοιπόν έναν τρόπο να μετράμε την <span class="key-term">ποιότητα μιας ζεύξης</span>. Ο απλός τρόπος: κάθε φορά που ένα πακέτο παραδίδεται επιτυχώς, ο παραλήπτης στέλνει πίσω ένα σύντομο μήνυμα επιβεβαίωσης, το λεγόμενο Acknowledgement (ACK). Μετρώντας πόσο συχνά φτάνει το ACK, ξέρουμε πόσο αξιόπιστη είναι η ζεύξη (Διάλεξη 4, διαφάνεια 7).
...
Μια μετάδοση θεωρείται πετυχημένη μόνο αν συμβούν **και τα δύο**: φτάσει το πακέτο και γυρίσει το ACK. Αν κάθε μετάδοση είναι μια ανεξάρτητη δοκιμή τύπου «πέτυχε ή απέτυχε», τότε ο <span class="key-term">αναμενόμενος αριθμός μεταδόσεων</span> ώσπου να περάσει ένα πακέτο είναι:
...
Η απάντηση είναι ότι το ACK μπαίνει στο μοντέλο ως **πιθανότητα, όχι ως ενέργεια**. Το \(d_r\), η πιθανότητα της επιστροφής, ζει μέσα στο ETX (θυμηθείτε, \(\text{ETX} = 1/(d_f \cdot d_r)\)): μια κακή διαδρομή επιστροφής ανεβάζει το ETX, άρα χρειάζονται περισσότερα ταξίδια **του πακέτου**, και αυτά είναι που κοστίζουν. Έτσι το δεύτερο ταξίδι επηρεάζει το **πόσες φορές** στέλνουμε, όχι το κόστος της κάθε φοράς. Η ενέργεια που θα ξόδευε το ίδιο το ACK αγνοείται, αφού ένα ACK είναι ελάχιστο, λίγα bits μπροστά στα 200 του πακέτου.
...
!!! warning "Σημείωση για την πηγή: αυτό δεν το δηλώνει ρητά το μάθημα"

    Οι διαφάνειες δίνουν τον τύπο και τον υπολογίζουν, αλλά **πουθενά δεν λένε** ότι αγνοείται η ενέργεια του ACK, ούτε ότι το ACK μετρά μόνο ως πιθανότητα. Τα παραπάνω είναι ερμηνεία του τι μετρά ο τύπος. Δεν είναι όμως αυθαίρετη: ο τύπος είναι το γνωστό **ραδιομοντέλο πρώτης τάξης** (first-order radio model) της βιβλιογραφίας των δικτύων αισθητήρων, το ίδιο πάνω στο οποίο στηρίχθηκε και το πρωτόκολλο του [Κεφαλαίου 2](02-leach.md). Είναι γραμμένο για το **πακέτο δεδομένων** και μόνο: κόστος \(E_c\) στα ηλεκτρονικά (πομπού και δέκτη) και \(E_{tx}\) στον ενισχυτή, χωρίς κανέναν όρο για ACK. Άρα το «αγνοούμε την ενέργεια του ACK» είναι ιδιότητα του **καθιερωμένου μοντέλου**, όχι ρητή επιλογή της διάλεξης. (Επαληθεύτηκε από εξωτερική βιβλιογραφία, εκτός των υλικών του μαθήματος.)
