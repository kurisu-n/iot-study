...
Μία **μεμονωμένη προσπάθεια** μετάδοσης ενός πακέτου b bits σε μια ζεύξη μήκους d κοστίζει, λοιπόν: \(E_c \cdot b\) στον πομπό (ραδιόφωνο) + \(E_{tx}(d) \cdot b\) στον πομπό (ενισχυτής) + \(E_c \cdot b\) στον δέκτη (ραδιόφωνο). Και επειδή χρειάζονται κατά μέσο όρο ETX προσπάθειες για να περάσει το πακέτο, πολλαπλασιάζουμε επί ETX:
