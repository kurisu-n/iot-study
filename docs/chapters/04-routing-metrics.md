...
Οι διαφάνειες δίνουν τον τύπο **χωρίς να σχολιάζουν** το ACK, οπότε το εξής είναι η ερμηνεία του τι μετρά ο τύπος, όχι κάτι που λέγεται ρητά. Κοιτάζοντας τους όρους, στο κόστος κάθε προσπάθειας μπαίνει μόνο το πάει του πακέτου· ενέργεια για το ACK δεν εμφανίζεται πουθενά. Αυτό στέκει, γιατί ένα ACK είναι **ελάχιστο** (λίγα bits, μπροστά στα 200 του πακέτου). Δεν χάνεται όμως το ACK από την εικόνα· μπαίνει ως **πιθανότητα, όχι ως ενέργεια**. Το \(d_r\), η πιθανότητα της επιστροφής, ζει μέσα στο ETX (θυμηθείτε, \(\text{ETX} = 1/(d_f \cdot d_r)\)): μια κακή διαδρομή επιστροφής ανεβάζει το ETX, άρα χρειάζονται περισσότερα ταξίδια **του πακέτου**, και αυτά είναι που κοστίζουν. Έτσι το δεύτερο ταξίδι επηρεάζει το **πόσες φορές** στέλνουμε, όχι το κόστος της κάθε φοράς.
